...
> 详细拆解：
>    - `sift_up()` 函数是一个死循环：从最后一个数（自己）开始，不断地和自己的parent级别打擂台，一直打到自己成为root(`while index > 0:`)，或者 parent 已经小于等于自己，(if self.data[parent]<= self.data[index]: break)，说明堆结构恢复了。
>    - `parent = (index - 1) // 2` 是一个寻找自己的parent的公式。（[点击查看：二叉树](binarytree.md)）
>    - `//` 是整除的意思，只要整数商，不要余数，不要小数点。
>    - 找到 parent 后，比较 parent 的值和自己的值(`if self.data[parent] <= self.data[index]:`)，parent 小于自己，说明已经到了最合适的位置，不需要再继续比较(`break`)。
...
> 详细解说
>    - 首先当数值交换之后，在外部的 scheduled queue 看来，数组中parent 的位置已经变了，外部的index也就变了。
...
>    - 如果我没有在王座上，那么上面和左右子打擂台，最后坐在王座上的某一子的数值和我的数值交换位置(`self.data[index], self.data[smallest] = self.data[smallest], self.data[index]`)。
...
其中，`*args` 是给 `callback` 准备的位置参数；`call_later()` 封装了 `call_at()`。
...
4. `heappush()` 内部根据 `TimerHandle.__lt__()` 做向上冒泡。
5. `timer._scheduled = True`。
6. `return timer`。

---

## 七、`_run_once()` 如何用堆顶决定等待时间

Event Loop 每转一圈，都会先看 `_ready` 里有没有马上可以执行的任务。

如果 `_ready` 里已经有任务，Event Loop 不会阻塞等待，而是用 `timeout = 0` 做一次非阻塞 selector 检查，然后继续处理 ready 回调。

如果 `_ready` 为空，但 `_scheduled` 里有定时任务，Event Loop 会看一眼堆顶，并计算：

```python
timeout = self._scheduled[0]._when - self.time()
```

`self._scheduled[0]` 就是最早到期的 TimerHandle。Event Loop 用它的 `_when` 减去当前时间，算出自己最多可以把当前线程交给底层 selector 等多久。

注意，这里等待的是 Event Loop 所在线程，不是某个具体 Task。Event Loop 是调度器；当暂时没有任务可调度时，它会让底层 selector 等待 I/O 事件或 timer 到期。

如果算出来的 timeout 小于等于 0，说明这个 timer 已经过期，Event Loop 就不会等待，会立刻回来处理它。

所以这一步的核心只有一句话：

`_scheduled[0]` 决定了 Event Loop 下一次最多可以让 selector 等多久。

如果 `_ready` 和 `_scheduled` 都是空的，Event Loop 就可以交给底层 selector 等待 I/O 事件或外部唤醒。
...
```
这里的 `self.time()` 是 Event Loop 使用的单调时钟时间；`self._clock_resolution` 是这个时钟的最小分辨率，也就是这个时钟能可靠区分的最小时间精度。

所以 `end_time` 的意思是：当前时间，再加上一点点“时钟精度缓冲”。

Event Loop 后面会把 `handle._when < end_time` 的 timer 从 `_scheduled` 搬进 `_ready`。这不是为了提前很多执行任务，而是为了处理时钟精度带来的微小误差。

- 这样做不是为了提前很多执行任务，而是为了处理时钟精度带来的微小误差：如果某个 timer 距离当前时间只差一个极小的时间单位，已经落在时钟分辨率范围内，Event Loop 就把它当作“已经到期或几乎到期”来处理。

`handle._scheduled = False` 不是取消 handle，而是更新它的队列状态。
- 前一行：`handle = heapq.heappop(self._scheduled)`已经把这个 `TimerHandle` 从 `_scheduled` 最小堆里弹出来了。既然它已经不在 scheduled queue 里面，就要把 `_scheduled` 标记改成 `False`。
- 所以这句的意思是：这个 `TimerHandle` 已经从 `_scheduled` 计划队列里拿出来了，不要再把它当成仍然排在 scheduled queue 里的 timer。

- 之后它会被追加到 _ready：`self._ready.append(handle)`
...
这让 Event Loop 的执行模型保持一致：不管是 `call_soon()`、I/O 回调，还是到期的 timer，最终都会变成 `_ready` 里的 `Handle`（封装），然后统一执行 `handle._run()`。
...
注意：这里先固定 `ntodo = len(self._ready)`。这意味着本轮执行过程中新增到 `_ready` 的回调（在`ntodo = len(self._ready)`后面新增的），不会在本轮继续执行，而是留到下一轮 `_run_once()`。
...
2. 定时任务会不断插入和到期弹出，所以插入、弹出都要稳定在 `O(log n)`。[点击查看：时间复杂度](time_complexity.md)
3. 不需要完整排序，只需要保证堆顶最小。
