...
## 2. 底层原理：分块数组链表 (Block-Linked Array)

`deque` 的底层数据结构是一个由固定大小数组（Block）组成的双向链式结构（unrolled linked list），同时也是一个 **“宏观靠指针关联，微观靠信标定位”** 的双层架构（在计算机科学中称为分块数组链表 Block-Linked Array）。

## 2.1.1 架构拆解：

### 微观层面（Block 内部）：
  - 每个 Block 内部是一个物理内存连续的数组，拥有固定的容量（具体容量属于 CPython 的实现细节，通常为几十个元素级别）。超出固定容量后，就会有新的Block产生。
  - 极其重要的一点：座位上存储的并不是 Python 对象本身，而是对象的内存地址（C 语言层面的指针 PyObject*）。这使得无论你放入多庞大的对象，每个座位都只占用固定的内存字节（比如64位系统占用8个字节），彻底消除了巨大对象的拷贝开销。
...
## 2.1.2 为什么不直接使用双向链表？
  ...
  - 因为链表的每个节点在内存中是离散分布的，会导致较差的缓存局部性（cache locality）。
  - deque 通过 block 内部的连续数组设计，在保持 O(1) 操作复杂度的同时，提高了 CPU 缓存命中率。
...
而在 deque 中，总控室通过精准控制两套信标，实现了 **“不挪数据，只平移信标”** 的神级操作：
  ...
  - 跨越 Block：当一节 Block 的元素全部出队走空时，deque 利用 Block 之间的 **“指针（虫洞）”**，顺藤摸瓜找到下一节 Block，并把自己的 **“宏观车厢信标（leftblock）”** 平移过去。旧的空 Block 可能被回收或复用（free list）。
...
## 总结
deque 的性能可以理解为：
  - **常态路径**：数组级 O(1)（index + 连续内存访问），
  - **边界路径**：链式结构 O(1)。（block 指针切换）
整体保持严格的摊销 O(1) 时间复杂度。